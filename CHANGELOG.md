...
- `README.md` and `values.schema.json` are now generated by pre-commit instead of being vendored or hand-maintained, so `sync.sh` and pre-commit no longer revert each other.
