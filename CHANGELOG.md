...
- changed: Harden the `securityContext` defaults so the chart satisfies Giant Swarm's Pod Security Standards (restricted) policies out of the box: non-root, no privilege escalation, all capabilities dropped, read-only root filesystem, and `RuntimeDefault` seccomp for both the Deployment and the `helm test` pod. The `values.schema.json` now rejects non-compliant overrides (`privileged: true`, `runAsUser: 0`, `runAsNonRoot: false`).
